...
function summarizeEvents(rows, days, nowMs = Date.now()) {
  ...
  for (const row of windowRows) {
    ...
    if (eventType === 'supporter_cta_clicked') daily.supporterClicks += 1;
  }

  const pageViews = counts.page_view;
  const reportsLoaded = counts.reports_loaded;
  const comparisons = counts.comparison_completed;
  const apiErrors = counts.api_error;
  const supporterCtaClicks = counts.supporter_cta_clicked;
  ...
  return {
    ...
    totals: {
      ...
      errorPerViewRate: pageViews > 0 ? Number((apiErrors / pageViews).toFixed(3)) : 0,
      errorPerLoadRate: reportsLoaded > 0 ? Number((apiErrors / reportsLoaded).toFixed(3)) : 0,
      supporterCtaClickRate: pageViews > 0 ? Number((supporterCtaClicks / pageViews).toFixed(3)) : 0,
    },
