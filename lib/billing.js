...
async function callUsageIncrementRpc(config, { userId, anonKey, date, dailyLimit }) {
  const res = await fetch(`${config.url}/rest/v1/rpc/check_and_increment_comparison_usage`, {
    method: 'POST',
...
async function checkAndIncrementUsage({ userId, anonKey }) {
  ...
  const date = todayJST();
  const rpcResult = await callUsageIncrementRpc(config, { userId, anonKey, date, dailyLimit });
