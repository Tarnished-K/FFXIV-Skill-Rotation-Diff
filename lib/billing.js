...
async function isUserPremium(userId) {
  ...
  url.searchParams.set('status', 'in.(active,trialing,past_due)');
...
async function checkAndIncrementUsage({ userId, anonKey }) {
  ...
  const config = getSupabaseConfig();
  if (!config) return { allowed: true, count: 0, remaining: DAILY_LIMIT, isPremium: false };

  if (userId) {
    const premium = await isUserPremium(userId);
    if (premium) return { allowed: true, count: 0, remaining: 9999, isPremium: true };
  }
  ...
  if (currentCount >= DAILY_LIMIT) {
    return { allowed: false, count: currentCount, remaining: 0, isPremium: false };
  }
  ...
  return { allowed: true, count: newCount, remaining: DAILY_LIMIT - newCount, isPremium: false };
}

async function getUsageStatus({ userId, anonKey }) {
  const DAILY_LIMIT = 30;
  const config = getSupabaseConfig();
  if (!config) return { count: 0, remaining: DAILY_LIMIT, isPremium: false };

  if (userId) {
    const premium = await isUserPremium(userId);
    if (premium) return { count: 0, remaining: 9999, isPremium: true };
  }
  ...
  if (userId) {
    ...
    return { count: 0, remaining: DAILY_LIMIT, isPremium: false };
  }
...
async function recordWebhookEvent({ stripeEventId, type, payload }) {
  ...
  if (res.status === 409) return { isDuplicate: true };
